...
| Bibliothèque | Recherche, listes, fiches, QCM et flashcards par deck |
...
- QCM et flashcards mettent à jour `recognized`.
...
La bêta Android `v2.0.0-beta.1` est publiée en FR uniquement.
L'ARB arabe reste conservé pour la suite, mais `ar` n'est pas exposé comme locale publique tant que la traduction n'est pas relue.
...
- ne pas réactiver `ar` dans `MaterialApp.supportedLocales` avant revue complète.
...
La clé de signature locale (`android/sirah-hub-release.jks`) et `android/key.properties` sont ignorés par Git et doivent être sauvegardés hors dépôt.
...
2. `DECISIONS.md`
3. `RELEASE.md`
4. `docs/JOURNEY_GUARDRAILS.md`
5. `docs/V2_1_AUDIT_BRIEF.md`
6. `docs/JOURNEY_RESTRUCTURE_PLAN.md`
7. docs V1/V1.5 uniquement pour contexte historique
